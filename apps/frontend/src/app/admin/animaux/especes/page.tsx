...
export default function EspecesPage() {
  ...
  const { data: species = [], isLoading } = useQuery<Species[]>({
    queryKey: ['species'],
    queryFn: () => api.get('/animaux-couvees/species').then(r => r.data),
  });

  const createMutation = useMutation({
    mutationFn: (data: any) => editSpecies
      ? api.put(`/animaux-couvees/species/${editSpecies.id}`, data)
      : api.post('/animaux-couvees/species', data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/animaux-couvees/species/${id}`),
