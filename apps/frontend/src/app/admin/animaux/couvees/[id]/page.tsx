...
export default function BroodDetailPage() {
  ...
  const { data: brood, isLoading } = useQuery({
    queryKey: ['brood', id],
    queryFn: () => api.get(`/animaux-couvees/couvees/${id}`).then(r => r.data),
  });

  const addEventMutation = useMutation({
    mutationFn: (data: any) => api.post(`/animaux-couvees/couvees/${id}/events`, data),
    onSuccess: () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (status: string) => api.put(`/animaux-couvees/couvees/${id}`, { status }),
