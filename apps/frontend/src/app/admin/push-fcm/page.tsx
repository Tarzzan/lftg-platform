    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          ...
          <p className={`text-sm font-semibold mt-1 ${vapidKey ?'text-green-400':'text-red-400'}`}>{vapidKey ?' Configurée':' Non configurée'}</p>
          {vapidKey && (
        ...
        <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
          ...
          <button
            ...
            {testMutation.isPending ?'Envoi...':' Envoyer test'}</button>
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
        ...
        <div className="space-y-3">
          <input
            type="text"placeholder="Titre de la notification *"value={broadcastForm.title}
          ...
          <textarea
            placeholder="Corps du message *"value={broadcastForm.body}
            onChange={(e) => setBroadcastForm({ ...broadcastForm, body: e.target.value })}
            rows={3}
            className="w-full bg-slate-700 text-white rounded-lg px-4 py-2 border border-slate-600 focus:outline-none focus:border-indigo-500"/>
          <input
            type="url"placeholder="URL de redirection (optionnel)"value={broadcastForm.url}
